 ...
 **/
@RestController
@RequestMapping("/zhangqiangsearch")
public class ZhangqiangSearchtest {

  @Autowired
  private RestHighLevelClient restHighLevelClient;


  //  http://localhost:8080/zhangqiangsearch/search123/{indexname}/{searchname}
  @RequestMapping("/search123/{indexname}/{searchname}")
  public ArrayList<Object> search123(@PathVariable String indexname,
      @PathVariable String searchname) throws IOException, InterruptedException {
    //1.往ceshi7索引中插入一条数据
      /*SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        BulkRequest bulkRequest = new BulkRequest();
         bulkRequest.add(new IndexRequest(indexname).id("1").source(JSON.toJSONString(
                new UserZhangqiang("1", "张强", 18,
                        ...
                        "供电服务指挥系统中配网23", "wangerxiao王二小names_pp_dd_hh_", new Date(),sdf.format(new Date()),"基于山东电网调度管理应用OMS系统中用户原因跳闸记录，结合营销应用业务系统中高压用户设备信息、供电服务指挥系统中配网设备参数信息，形成分析数据表，应用省公司自助式分析平台开展大数据挖掘分析。首先将相关数据进行数据清洗，处理异常值、缺失值，统一数据结构。其次通过柱状图、折线图、地理图等方式对客户原因造成的停电次数分布、停电时间点分布、累计停电时长、停电原因、停电设备的投运时间等开展数据可视化展示，初" )), XContentType.JSON));
        Thread.sleep(1);
        ...
        bulkRequest.add(new IndexRequest(indexname).id("5").source(JSON.toJSONString(
                new UserZhangqiang("5", "王二小", 100,
                        ...
                        "Brown rabbits are commonly seen.", "wangerxiao王二小names_pp_dd_hh_", new Date(),sdf.format(new Date()),"基于山东电网调度管理应用OMS系统中用户原因跳闸记录，结合营销应用业务系统中高压用户设备信息、供电服务指挥系统中配网设备参数信息，形成分析数据表，应用省公司自助式分析平台开展大数据挖掘分析。首先将相关数据进行数据清洗，处理异常值、缺失值，统一数据结构。其次通过柱状图、折线图、地理图等方式对客户原因造成的停电次数分布、停电时间点分布、累计停电时长、停电原因、停电设备的投运时间等开展数据可视化展示，初")), XContentType.JSON));
        Thread.sleep(1);
...
//        searchSourceBuilder.query(QueryBuilders.termQuery("age",18));
    ...
    searchSourceBuilder.query(
        QueryBuilders.boolQuery()
            ...
            .should(QueryBuilders.matchPhraseQuery("name.pinyin", searchword))
            .should(QueryBuilders.matchQuery("name.pinyin", searchword))

                  /*.should(QueryBuilders.wildcardQuery("title.keyword", "*" + searchword + "*"))
                        .should(QueryBuilders.matchQuery("title", searchword))
                        .should(QueryBuilders.matchPhraseQuery("title", searchword))
                        .should(QueryBuilders.matchPhraseQuery("title.pinyin",searchword))
                        .should(QueryBuilders.matchQuery("title.pinyin",searchword))*/

    );
       /* searchSourceBuilder.query(
                QueryBuilders.boolQuery()
                        .should(QueryBuilders.termQuery("name.keyword", "*" + searchword + "*"))



        );*/
    //加上keyword以后就可以不用拼音分词器了 不加则会给拼音分词掉
//        searchSourceBuilder.aggregation(AggregationBuilders.terms("paiming").field("name.keyword").order(BucketOrder.count(false)));

    //按照插入的  zhangqiangdate 字段进行降序排序
//        searchSourceBuilder.sort(new FieldSortBuilder("birth").order(SortOrder.ASC));
//        searchSourceBuilder.sort("age",SortOrder.DESC);
    //根据名字降序//根据创建时间排序降序
//        searchSourceBuilder.sort(new FieldSortBuilder("age").order(SortOrder.DESC));
//        searchSourceBuilder.sort(new FieldSortBuilder("zhangqiangdate").order(SortOrder.DESC));
   ...
   * @Return void
   **/
  //  http://localhost:8080/zhangqiangsearch/createindexSettingandinsertdatasssaaa/ceshi3/name,address,title,content
...
//        Long birth=1602494585771L;
    String current = System.currentTimeMillis() + "";
    System.out.println(current);
//        String dataes="1602495264585";
    SimpleDateFormat simpleDateFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss:ssss");
    String ss = simpleDateFormat.format(new Date(Long.valueOf(current)));
    System.out.println(ss);

    String zhuanhuan = String.valueOf(simpleDateFormat.parse(ss).getTime());
    System.out.println(zhuanhuan);
    System.out.println(simpleDateFormat.parse(ss).getTime());

    String sssd = simpleDateFormat.format(new Date(Long.valueOf("1602494585763")));
...
//        searchSourceBuilder.aggregation(AggregationBuilders.terms("zhangqiang").field("age"));
    ...
    searchSourceBuilder.aggregation(
        ...
        AggregationBuilders.terms("zhangqiang").field("age")
            .subAggregation(
                AggregationBuilders.avg("avgsss").field("age")/*.order(BucketOrder.count(true)*/)
    );
